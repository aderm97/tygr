...
TYGR_IMAGE = os.getenv("TYGR_IMAGE", "tygr-security-platform:latest")
...
class DockerRuntime(AbstractRuntime):
    ...
    def _create_container_with_retry(self, scan_id: str, max_retries: int = 3) -> Container:
        last_exception = None
        container_name = f"tygr-scan-{scan_id}"

        for attempt in range(max_retries):
            try:
                self._verify_image_available(TYGR_IMAGE)
    ...
    def _get_or_create_scan_container(self, scan_id: str) -> Container:  # noqa: PLR0912
        ...
        try:
            ...
            if (
                "tygr-scan-id" not in container.labels
                or container.labels["tygr-scan-id"] != scan_id
            ):
                logger.warning(
        ...
        try:
            containers = self.client.containers.list(
                all=True, filters={"label": f"tygr-scan-id={scan_id}"}
            )
            if containers:
    ...
    def _initialize_container(
        ...
        container.exec_run(
            ...
            f"TYGR_SANDBOX_MODE=true CAIDO_API_TOKEN={caido_token} CAIDO_PORT={caido_port} "
            f"poetry run python tygr/runtime/tool_server.py --token {tool_server_token} "
