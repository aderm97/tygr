...
if TYPE_CHECKING:
    from tygr.telemetry.tracer import Tracer
...
from tygr.llm import LLM, LLMConfig, LLMRequestFailedError
from tygr.llm.utils import clean_content
from tygr.tools import process_tool_invocations
...
class BaseAgent(metaclass=AgentMeta):
    ...
    def _add_to_agents_graph(self) -> None:
        from tygr.tools.agents_graph import agents_graph_actions
    ...
    async def _wait_for_input(self) -> None:
        ...
        if self.state.has_waiting_timeout():
            ...
            try:
                from tygr.tools.agents_graph.agents_graph_actions import _agent_graph
    ...
    async def _initialize_sandbox_and_state(self, task: str) -> None:
        ...
        sandbox_mode = os.getenv("TYGR_SANDBOX_MODE", "false").lower() == "true"
        if not sandbox_mode and self.state.sandbox_id is None:
            from tygr.runtime import get_runtime
    ...
    def _check_agent_messages(self, state: AgentState) -> None:  # noqa: PLR0912
        try:
            from tygr.tools.agents_graph.agents_graph_actions import _agent_graph, _agent_messages
